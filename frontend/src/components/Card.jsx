  ...
  return (
    <div className="max-w-sm w-full bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 border border-gray-300" onClick={()=>navigate(`/viewcourse/${id}`)}>
      ...
      <div className="p-5 space-y-2">
        {/* Title */}
        <h2 className="text-lg font-semibold text-gray-900 cursor-pointer">{title}</h2>

        {/* Category */}
        <span className="px-2 py-0.5 bg-gray-100 rounded-full text-gray-700 capitalize cursor-pointer">
            {category}
          </span>
        ...
        <div className="flex justify-between text-sm text-gray-600 mt-3 px-[10px]">
          ...
          <span className="font-semibold text-gray-800 cursor-pointer">₹{price}</span>
         
           <span className="flex items-center gap-1 cursor-pointer ">
            <FaStar className="text-red-500" /> {avgRating}
          </span>
